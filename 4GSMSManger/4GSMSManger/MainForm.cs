...
namespace _4GSMSManger
{
    public partial class MainForm : Form
    {
        ...
        SQLiteConnection dbconn_providers;
        bool is_providers = true;
        string location = "";
        ...
        public MainForm()
        {
            ...
            dbconn_providers = createProvidersConnection();
            InitializeComponent();
            showInputLocation();
        }
        ...
        private SQLiteConnection createConnection()
        {
            ...
            try
            {
                ...
                string sql = "CREATE TABLE IF NOT EXISTS \"main\".\"4GSMS\"(\"ID\" INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, \"DATE\" TEXT, \"TIME\" TEXT, \"BBU\" text, \"PROVIDER\" text, \"IMSI\" text, \"IMEI\" TEXT, \"COUNT\" INTEGER, \"RSSI\" TEXT, \"LOCATION\" TEXT)";
                sqlite_cmd = sqlite_conn.CreateCommand();
                sqlite_cmd.CommandText = sql;
                sqlite_cmd.ExecuteNonQuery();

                sql = "CREATE INDEX IF NOT EXISTS \"main\".\"index_imsi\" ON \"4GSMS\"( \"IMSI\")";
                sqlite_cmd.CommandText = sql;
                sqlite_cmd.ExecuteNonQuery();
            }
            catch (Exception ex) { }
        ...
        private void insertDb(string bbu, string provider, string imsi, string imei, int count, string rssi, string country)
        {
            try
            {
                SQLiteCommand sqlite_cmd = dbconn.CreateCommand();
                sqlite_cmd.CommandText = "INSERT INTO \"main\".\"4GSMS\"(\"DATE\", \"TIME\", \"BBU\", \"PROVIDER\", \"IMSI\", \"IMEI\", \"COUNT\", \"RSSI\", \"LOCATION\") VALUES (date('now', 'localtime'), time('now', 'localtime'), '" + bbu + "', '" + provider + "', '" + imsi + "', '" + imei + "', @count, '" + rssi + "', '" + location + "')";
                sqlite_cmd.Parameters.Add(new SQLiteParameter("@count", count));
                sqlite_cmd.ExecuteNonQuery();
            }
            catch (Exception ex) {}
        }
        private void updateDb(String bbu, String provider, String imsi, String imei, int count, String rssi, string country)
        {
            try
            {
                SQLiteCommand sqlite_cmd = dbconn.CreateCommand();
                sqlite_cmd.CommandText = "UPDATE \"main\".\"4GSMS\" SET \"DATE\"=date('now', 'localtime'), \"TIME\"=time('now', 'localtime'), \"BBU\"='" + bbu + "', \"PROVIDER\"='" + provider + "', \"IMEI\"='" + imei + "', \"COUNT\"=@count, \"RSSI\"='" + rssi + "', \"LOCATION\"='" + location + "' WHERE \"IMSI\"='" + imsi + "'";
                sqlite_cmd.Parameters.AddWithValue("@count", count);
                sqlite_cmd.Parameters.AddWithValue("@location", location + ", " + country);
                sqlite_cmd.ExecuteNonQuery();
            }
            catch (Exception ex) { }
        }

        private SQLiteConnection createProvidersConnection()
        {
            SQLiteConnection sqlite_conn;
            sqlite_conn = new SQLiteConnection("Data Source=providers.db;Version=3;New=True;Compress=True;");
            try
            {
                sqlite_conn.Open();
            }
        ...
        private string getProviderLocation(string imsi, ref string providerName, ref string country)
        {
            string mccmnc = "";
            providerName = "";
            country = "";
            try
            {
                if (is_providers)
                {
                    ...
                    string mnc1 = imsi.Substring(3, 3);
                    string mnc2 = imsi.Substring(3, 2);
                    SQLiteDataReader reader;
                    SQLiteCommand sqlite_cmd;
                    sqlite_cmd = dbconn_providers.CreateCommand();
                    sqlite_cmd.CommandText = "SELECT \"provider\", \"country\" FROM \"main\".\"providers\" WHERE \"mcc\"=@mcc and \"mnc\"=@mnc";
                    sqlite_cmd.Parameters.AddWithValue("@mcc", mcc);
                    sqlite_cmd.Parameters.AddWithValue("@mnc", mnc1);
                    ...
                    reader.Close();
                    if (providerName == "" && int.Parse(mnc1) != int.Parse(mnc2))
                    {
                        SQLiteCommand sqlite_cmd1;
                        sqlite_cmd1 = dbconn_providers.CreateCommand();
                        sqlite_cmd1.CommandText = "SELECT \"provider\", \"country\" FROM \"main\".\"providers\" WHERE \"mcc\"=@mcc and \"mnc\"=@mnc";
                        sqlite_cmd1.Parameters.AddWithValue("@mcc", mcc);
                        sqlite_cmd1.Parameters.AddWithValue("@mnc", mnc2);
                        reader = sqlite_cmd1.ExecuteReader();
                        while (reader.Read())
                        {
                            providerName = reader.GetString(0);
                            country = reader.GetString(1);
                ...
                {
                    providerName = imsi.Substring(0, 5);
                    mccmnc = providerName;
                }
            }
            catch (Exception ex)
        ...
        void addGraphData(string mccmnc)
        {
            int index = numList.IndexOf(mccmnc);
            if (index >= 0)
            {
                barChart.Series[0].Points[index].YValues[0] = barChart.Series[0].Points[index].YValues[0] + 1;
                pieChart.Series[0].Points[index].YValues[0] = pieChart.Series[0].Points[index].YValues[0] + 1;
            }
            else
            {
                barChart.Series[0].Points.AddXY(barChart.Series[0].Points.Count(), 1);
                numList.Add(mccmnc);
                CustomLabel label = new CustomLabel();
                label.Text = mccmnc;
                label.FromPosition = barChart.Series[0].Points.Count() - 1.5;
                label.ToPosition = barChart.Series[0].Points.Count() - 0.5;
                label.ForeColor = Color.Black;
                barChart.ChartAreas[0].AxisX.CustomLabels.Add(label);
                pieChart.Series[0].Points.AddXY(mccmnc, 1);
            }
            barChart.Invalidate();
            pieChart.Invalidate();
        }
        ...
        private void showInputLocation()
        {
            ...
            inputDialog.Text = "Input Location";
            ...
            textBox.Location = new System.Drawing.Point(10, 36);
            ...
            okButton.Text = "OK";
            okButton.Location = new System.Drawing.Point(20, 75);
            ...
            cancelButton.Location = new System.Drawing.Point(180, 75);
            ...
            if (result == DialogResult.OK)
            {
                ...
                {
                    MessageBox.Show("Location must not be null string.");
                ...
                case ENDPOINT_4G_SWEEP_MODULAR:
                    {
                        ...
                        if (buffer[0] == 1 || buffer[0] == 2)
                        {
                            if (buf_4g_msg != "")
                            {
                                var thread = new Thread(() => { PciScan(buf_4g_msg); });
                                thread.Start();
                                thread.Join();
                                thread = null;
                        ...
                        else // AT command
                        {
                            ...
                            if (startIdx < lastIdx)
                            {
                                ...
                                {
                                    var thread = new Thread(() => { PciScan(atCmd); });
                                    thread.Start();
                                    thread.Join();
                                    thread = null;
                                }
                ...
                case ENDPOINT_2G_SWEEP_MODULAR:
                    {
                        ...
                        if (buffer[0] == 1 || buffer[0] == 2)   // General Message
                        {
                            if (buf_2g_msg != "")
                            {
                                var thread1 = new Thread(() => { GSMScan(buf_2g_msg); });
                                thread1.Start();
                                thread1.Join();
                                thread1 = null;
                                buf_2g_msg = "";
                            ...
                            var thread = new Thread(() => { GSMScan(msg); });
                            thread.Start();
                            thread.Join();
                            thread = null;
                        }
                        else    // AT command
                        {
                            ...
                            if (startIdx < lastIdx)
                            {
                                ...
                                var thread = new Thread(() => { GSMScan(atCmd); });
                                thread.Start();
                                thread.Join();
                                thread = null;
        ...
        public void Lte_imsi(byte[] data, int length)
        {
            ...
            if (data[1] == 0x13)
            {
                for (int i = 0; i < data[14]; i++)
                {
                    ...
                    var thread = new Thread(() => { DateWrite_4G(parameter); });
                    thread.Start();
                    thread.Join();
                    thread = null;
                }
        ...
        public void DateWrite_4G(object str)
        {
            InvokeUI(() =>
            {
                ...
                while (str2.Length > 0 && !(Encoding.ASCII.GetBytes(str2)[0] >= 0x30 && Encoding.ASCII.GetBytes(str2)[0] <= 0x39))
                ...
                while (true)
                {
                    if (num2 < count)
                    {
                        if (this.imsiimei.Rows[num2].Cells[1].Value.ToString() != str2)
                        {
                            num2++;
                            continue;
                        }
                        int newCount = Convert.ToInt32(this.imsiimei.Rows[num2].Cells[6].Value) + 1;
                        this.imsiimei.Rows[num2].Cells[6].Value = (newCount).ToString();
                        this.imsiimei.Rows[num2].Cells[5].Value = DateTime.Now.ToString();
                        string provider = "";
                        string country = "";
                        string mccmnc = getProviderLocation(str2, ref provider, ref country);
                        updateDb("4G", provider, str2, "", newCount, "", country);
                        addGraphData(provider);
                    ...
                    if (flag)
                    {
                        ...
                        this.imsiimei.Rows[num4].Cells[1].Value = str2;
                        this.imsiimei.Rows[num4].Cells[2].Value = "";
                        this.imsiimei.Rows[num4].Cells[3].Value = "OK";
                        this.imsiimei.Rows[num4].Cells[4].Value = "";
                        this.imsiimei.Rows[num4].Cells[5].Value = DateTime.Now.ToString();
                        this.imsiimei.Rows[num4].Cells[6].Value = "1";
                        this.imsiimei.Rows[num4].Cells[7].Value = "4G";
                        this.imsiimei.Rows[num4].Cells[8].Value = provider;
                        this.imsiimei.Rows[num4].Cells[9].Value = country;
                        this.imsiimei.Rows[num4].Cells[10].Value = location;
                        insertDb("4G", provider, str2, "", 1, "", country);
                        addGraphData(provider);
                        Console.Beep(5000, 300);
                    }
                    return;
        ...
        private void Gsm1Receiveparameter(byte[] arryHex)
        {
            ...
            if (arryHex[5] == 0x18)
            {
                ...
                for (int i = 0; i < arryHex.Length; i++)
                {
                    ...
                    {
                        var thread = new Thread(() => DateWrite(string.Concat(new object[] { str, ",", str2, ",", (Convert.ToInt16(arryHex[i + 4]) - 0xff).ToString(), ",GSM1" })));
                        thread.Start();
                        thread.Join();
                        thread = null;
                    }
        ...
        public void DateWrite(object str)
        {
            InvokeUI(() =>
            {
                ...
                bool flag = true;
                int num2 = 0;
                string imsi = strArray[0].ToString();
                string imei = strArray[1].ToString();
                while (imsi.Length > 0 && !(Encoding.ASCII.GetBytes(imsi)[0] >= 0x30 && Encoding.ASCII.GetBytes(imsi)[0] <= 0x39))
                    imsi = imsi.Substring(1);
                while (imei.Length > 0 && !(Encoding.ASCII.GetBytes(imei)[0] >= 0x30 && Encoding.ASCII.GetBytes(imei)[0] <= 0x39))
                ...
                while (true)
                {
                    if (num2 < count)
                    {
                        if (this.imsiimei.Rows[num2].Cells[1].Value.ToString() != imsi)
                        {
                            num2++;
                            continue;
                        }
                        ...
                        string provider = "";
                        string country = "";
                        string mccmnc = getProviderLocation(imsi, ref provider, ref country);
                        updateDb(strArray[3], provider, imsi, imei, newCount, strArray[2], country);
                        addGraphData(provider);
                    ...
                    if (flag)
                    {
                        ...
                        this.imsiimei.FirstDisplayedScrollingRowIndex = num4;
                        this.imsiimei.Rows[num4].Cells[0].Value = num4;
                        this.imsiimei.Rows[num4].Cells[1].Value = imsi;
                        this.imsiimei.Rows[num4].Cells[2].Value = imei;
                        this.imsiimei.Rows[num4].Cells[3].Value = "OK";
                        ...
                        insertDb(strArray[3], provider, imsi, imei, 1, strArray[2], country);
                        addGraphData(provider);
        ...
        private void Gsm2Receiveparameter(byte[] arryHex)
        {
            ...
            if (arryHex[5] == 0x18)
            {
                ...
                for (int i = 0; i < arryHex.Length; i++)
                {
                    ...
                    {
                        var thread = new Thread(() => DateWrite(string.Concat(new object[] { str, ",", str2, ",", (Convert.ToInt16(arryHex[i + 4]) - 0xff).ToString(), ",GSM2" })));
                        thread.Start();
                        thread.Join();
                        thread = null;
                    }
        ...
        {
            //ArrayList ips = Utils.GetIPAddressList();

            //cmbIPAddress.Items.Add("Any");
            //foreach (var ip in ips)
            //{
            //    IPAddress ipAddr = (IPAddress)ip;
        ...
        private void btnCSV_Click(object sender, EventArgs e)
        {
            ...
            saveFileDialog1.Title = "Browse CSV Files";
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    int count = this.imsiimei.Rows.Count;
                    String header = "\"ID\",\"IMSI\",\"IMEI\",\"STATUE\",\"RSSI\",\"TIME\",\"COUNT\",\"BBU\",\"PROVIDER\",\"COUNTRY\",\"LOCATION\"\n";
                    sw.Write(header);
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        for (int j = 0; j < 11; j++)
                        {
                            row += "\"" + this.imsiimei.Rows[i].Cells[j].Value.ToString() + "\"";
                            if (i != 10)
                                row += ",";
                        }
                        row += "\n";
